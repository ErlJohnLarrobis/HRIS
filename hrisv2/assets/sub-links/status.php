        ...
        <section class="content" style="height: 100vh;">
            <div class="header bg-primary p-3">
                ...
                <ul class="nav nav-pills">
                    <li>
                        <a href="/hrisv2/assets/dropdown-files/new-applicant.php" class="nav-link text-white">New Applicant</a>
                    </li>
                    <li>
                    ...
                    <li>
                        <a href="/hrisv2/assets/sub-links/status.php" class="nav-link bg-white">Status</a>
                    </li>
                </ul>
            </div>
            ...
            <?php 
                        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
                            if (isset($_POST['rowId'])) {
                                $id = $_POST['rowId'];
                                $utitle = $_POST['update-title'];
                                $sqlupdate = "UPDATE `career_applicants_status` SET `astatus_title`='$utitle' WHERE astatus_no=$id";
                                ...
                                $id_delete = $_POST["delete-button"];
        
                                // Delete employee from table
                                $sql = "UPDATE career_applicants_status SET astatus_delete=1 WHERE astatus_no=$id_delete";
                ...
                <form
                    ...
                    <div class="mb-3 col-8">
                        <label for="contact" class="form-label">Title:</label>
                            <input type="text" class="form-control" required="required" onkeyup="searchTable()" id="myInput">
                            <div class="invalid-feedback">Please provide a valid Title !</div>
                    </div>
                    <div class="mb-3 col-8">
                    <button class="btn btn-primary rounded-end" onclick="searchTable()">View</button>
                    </div>
                </form>
                <div id="myModal1" class="thismodal" style="display:none;">
                    <div class="modal-content">
                        <form class="row justify-content-center" action="<?php echo $_SERVER['PHP_SELF']; ?>"  method="post">
                            <div class="mb-3 col-8">
                                <label>Update</label>
                                <input class="form-control" type="text" name="update-title">
                                <input class="form-control" type="hidden" id="rowId" name="rowId">
                            </div>
                            <div class="mb-3 col-8">
                ...
                <div class="table">
                    ...
                    <table class="table mt-5" id='myTable'>
                        ...
                        <tbody>
                            <?php
                            $sql = "SELECT * FROM career_applicants_status WHERE astatus_delete=0";
                            $result = $conn->query($sql);
                            while($row = mysqli_fetch_assoc($result)):
                            ?>
                            <tr class="clearfix" data-row-id="<?php echo htmlentities($row['astatus_no']) ?>">
                            <td class="float-start"><?php echo htmlentities($row['astatus_title'])?></td>
                            <td class="d-flex float-end">
                                <button class="btn btn-info update-button" type="button" data-bs-toggle="modal" data-bs-target="#myModal" data-bs-whatever="@mdo">Edit</button>
                                <form method='post' action='<?php echo $_SERVER['PHP_SELF']; ?>'><input type='hidden' name='delete-button' value='<?php echo $row['astatus_no'] ?>'><input class="btn btn-danger" type='submit' value='Delete'></form>
                            </td>
                            </tr>
                            <?php endwhile ?>
                        </tbody>
...
function searchTable() {
  // Get the input value and the table
