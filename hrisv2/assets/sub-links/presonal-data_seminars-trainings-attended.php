            ...
            <div class="container bg-light shadow-lg rounded py-3" style="margin-top: -220px; min-height: 220px;">
                ...
                ?>
                    <ul class="nav nav-pills nav-fill gap-2">
                        ...
                        <li class="nav-item border border-primary rounded">
                            <a href="/hrisv2/assets/sub-links/presonal-data_seminars-trainings-attended.php?emp_id=<?php echo $id?>" class="nav-link active">Seminars or Trainings Attended</a>
                        </li>
                        <li class="nav-item border border-primary rounded">
                ...
                <hr>
                <?php 
                $sql1= "SELECT * FROM users LEFT JOIN employee_seminars ON users.user_no = employee_seminars.user_no LEFT JOIN seminar_types ON employee_seminars.seminar_no = seminar_types.seminar_no WHERE user_id='22-0001' AND es_delete=0;";
                $res=$conn->query($sql1);
                while($row1=$res->fetch_assoc()):
                ?>
                <form class="row justify-content-center needs-validation mb-5">
                    <div class="mb-3 col-8">
                        <label for="simpleinput" class="form-label">Seminar/Training Title:</label>
                        <input type="text" class="form-control" value="<?php echo $row1['es_title']?>" required>
                    </div>
                    <div class="mb-3 col-8">
                        <label for="simpleinput" class="form-label">Venue:</label>
                        <input type="text" class="form-control" value="<?php echo $row1['es_venue']?>" required>
                    </div>
                    <div class="mb-3 col-8">
                        <label for="simpleinput" class="form-label">Period:</label>
                        <input type="text" class="form-control" value="<?php echo $row1['es_start']?>" placeholder="start" disabled>
                        <input type="text" class="form-control" value="<?php echo $row1['es_end']?>" placeholder="end" disabled>
                    </div>
                    <div class="mb-3 col-8">
                        <label for="simpleinput" class="form-label"># of Hours:</label>
                        <input type="text" class="form-control" value="<?php echo $row1['es_hours']?>" required>
                    </div>
                    <div class="mb-3 col-8">
                        <label for="simpleinput" class="form-label">Type of LD:</label>
                        <div class="input-group">
                            <select class="form-select" id="salary-grade" name="position" required>
                                <option selected disabled  value="<?php echo $row1['seminar_name']?>"><?php echo $row1['seminar_name']?></option>
                                <?php
                                    $result = mysqli_query($conn, "SELECT * FROM seminar_types WHERE seminar_delete=0");
                                    while ($row = mysqli_fetch_assoc($result)) {
                                        $select = $row["seminar_name"];
                                        echo "<option value=\"$select\">$select</option>";
                                    }
                                    ?>
                            </select>
                            <button type="button" class="btn btn-danger"><i class="bi bi-plus-lg"></i></button>
                        </div>
                    </div>
                    <div class="mb-3 col-8">
                        <label for="simpleinput" class="form-label">Conducted/Sponsored by:</label>
                        <input type="text" class="form-control" value="<?php echo $row1['es_conducted']?>" required>
                    </div>
                    <div class="mb-3 col-8">
                        <label for="simpleinput" class="form-label">Return of Service:</label>
                        <input type="text" class="form-control" value="<?php echo $row1['es_service']?>" required>
                    </div>
                    <div class="mb-3 col-8">
                        <label for="simpleinput" class="form-label">Remark:</label>
                        <input type="text" class="form-control" value="<?php echo $row1['es_remark']?>" required>
                    </div>
                    <div class="mb-3 col-8">
                ...
                <div class="table">
                    <label>Seminars/Trainings Details</label>
                    <table id="data-table" class="table table-hover mt-5">
                        ...
                        <tbody>
                            <td><?php echo $row1['es_title']?></td>
                            <td><?php echo $row1['es_start'].'/'.$row1['es_end']?></td>
                            <td><?php echo $row1['es_hours']?></td>
                            <td><?php echo $row1['seminar_name']?></td>
                            <td><?php echo $row1['es_conducted']?></td>
                            <td><?php echo $row1['es_service']?></td>
                            <td><?php echo $row1['es_remark']?></td>
                        </tbody>
                    </table>
                </div>  
                <?php endwhile?> 
