            ...
            <div class="container bg-light shadow-lg rounded py-3">
                <div class="table">
                    <table id="data-table" class="table table-hover mt-5">
                        ...
                        <tbody>
                            <?php
                            $sql = "SELECT sg_no, GROUP_CONCAT(sgs_amount ORDER BY sgs_number SEPARATOR ', ') AS combined_data FROM `salary_grades_steps` GROUP BY sg_no;";

                            $result = $conn->query($sql);
                            $prev_column1 = null;
                            while ($row = $result->fetch_assoc()) {
                                echo '<tr>';
                                echo '<td>' . $row['sg_no'] . '</td>';
                                // Split the concatenated data into an array
                                $values = explode(', ', $row['combined_data']);
                                // Loop through the array and output each value as a separate column
