        ...
        <section class="content" style="height: 100vh;">
            <div class="header bg-primary p-3">
                ...
                <ul class="nav nav-pills">
                    ...
                    <li>
                        <a href="/hrisv2/assets/sub-links/plantilla-sg-table.php" class="nav-link bg-white">SG Table</a>
                    </li>
                    <li>
            ...
            <div class="container bg-light shadow-lg rounded py-3">
                ...
                <table class="table table-hover">
                    <thead class="text-white bg-primary sticky-top">
                        <th>Name</th>
                        <?php
                        $sql = "SELECT DISTINCT sgs_order FROM `salary_grades_steps` WHERE sgs_delete=0 ORDER BY sgs_order;";
                        $result = $conn->query($sql);
                        while ($row = $result->fetch_assoc()) {
                        echo '<th>' . $row['sgs_order'] . '</th>';
                        }
                        ?>
                    </thead>
                    <tbody>
                        <?php
                        $sql = "SELECT sg_no, MAX(sgs_number) as sgs_number, MAX(sgs_amount) as sgs_amount FROM salary_grades_steps GROUP BY sg_no;";
                        $result = $conn->query($sql);

                        while($row = mysqli_fetch_assoc($result)) { 
                        $sg_no = $row['sg_no'];
                        $sgs_amounts = array();
                        $sql2 = "SELECT * FROM salary_grades_steps WHERE sg_no = '$sg_no'";
                        $result2 = mysqli_query($conn, $sql2);
                        while($row2 = mysqli_fetch_assoc($result2)) {
                            $sgs_number = $row2['sgs_number'];
                            $sgs_amounts[$sgs_number] = $row2['sgs_amount'];
                        }
                        ?>
                        <tr>
                        <td><?php echo $sg_no; ?></td>
                        <?php
                        foreach ($sgs_amounts as $key => $value) {
                            if (!empty($value)) {
                                echo '<td>'.$value.'</td>';
                            } else {
                                echo '<td> </td>';
