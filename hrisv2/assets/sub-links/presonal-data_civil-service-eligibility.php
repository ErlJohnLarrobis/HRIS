            ...
            <div class="container bg-light shadow-lg rounded py-3" style="margin-top: -220px; min-height: 220px;">
                ...
                if ($_SERVER['REQUEST_METHOD'] == 'POST') {
                    ...
                    if(empty($_POST['date'])){
                        ...
                        $elig_date = $_POST['date'];
                    }
                    if(empty($_POST['validity'])){
                        $elig_validity = NULL;
                    }else{
                        $elig_validity = $_POST['validity'];
                    }

                    $sql_elig = "SELECT eligibility_no FROM eligibilities WHERE eligibility_title = '$category'";
                    $result_elig = mysqli_query($conn, $sql_elig);
                    if (mysqli_num_rows($result_elig) > 0) {
                    $row_elig = mysqli_fetch_assoc($result_elig);
                    $elig_no = $row_elig['eligibility_no'];
                    } else {
                    die("level not found");
                    }

                    $sqlupdate="INSERT INTO `employee_eligibility` (`eligibility_no`, `eelig_title`, `eelig_rating`, `eelig_date`, `eelig_address`, `eelig_license`, `eelig_license_expiry`, `eelig_encode_date`, `user_no`)
                    SELECT '$elig_no','$title','$rating','$elig_date','$place','$license','$elig_validity',NOW(), b.user_no 
                    FROM `users` b
                    WHERE b.user_id='$id'";
                ...
                <form class="row justify-content-center needs-validation mb-5" method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]) . '?emp_id=' . $id;?>">
                        ...
                        <div class="mb-3 col-8">
                            ...
                            <input type="text" class="form-control" name="validity" placeholder="Date of Validity" required disabled>
                        </div>
                        <div class="mb-3 col-8">
