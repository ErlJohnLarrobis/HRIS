            ...
            <div class="container bg-light shadow-lg rounded py-3" style="margin-top: -220px; min-height: 220px;">
                ...
                ?>
                    <ul class="nav nav-pills nav-fill gap-2">
                        ...
                        <li class="nav-item border border-primary rounded">
                            <a href="/hrisv2/assets/sub-links/presonal-data_group-affiliations.php?emp_id=<?php echo $id?>" class="nav-link  active">Group Affiliations</a>
                        </li>
                        <li class="nav-item border border-primary rounded">
                ...
                if($_SERVER['REQUEST_METHOD'] == 'POST'){
                    $org = $_POST['organization-name'];
                    $position = $_POST['position'];

                    $sql_org = "SELECT orga_no FROM organizations WHERE orga_title = '$org'";
                    $result_org = mysqli_query($conn, $sql_org);
                    if (mysqli_num_rows($result_org) > 0) {
                    $row_org = mysqli_fetch_assoc($result_org);
                    $org_no = $row_org['orga_no'];
                    } else {
                    die("Organizations not found");
                    }

                    $sqlinsert = "INSERT INTO `employee_organization`(`orga_no`,`org_position`, `org_encode_date`,`user_no`) 
                    SELECT '$org_no','$position',NOW(), b.user_no 
                    FROM `users` b
                    WHERE b.user_id='$id'";
                ...
                <form class="row justify-content-center needs-validation mb-5" method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]) . '?emp_id=' . $id;?>">
                    <div class="mb-3 col-8">
                        ...
                        <div class="input-group">
                            <select class="form-select" id="salary-grade" name="organization-name" required>
                                <option selected disabled value="">Select</option>
                                <?php
                                $result = mysqli_query($conn, "SELECT * FROM organizations");
                                while ($row = mysqli_fetch_assoc($result)) {
                                    $position = $row["orga_title"];
                ...
                <div class="table">
                    <table id="data-table" class="table table-hover mt-5">
                        ...
                        <?php 
                        $sql1 ="SELECT * FROM users a LEFT JOIN employee_organization b ON a.user_no = b.user_no LEFT JOIN organizations c ON b.orga_no = c.orga_no WHERE user_id='$id';";
                        $res1 = $conn->query($sql1);
                        while($row1=$res1->fetch_assoc()):
                        ?>
                        <tbody>
                            <td><?php echo $row1['orga_title']?></td>
                            <td><?php echo $row1['org_position']?></td>
                            <td><?php echo trim($row1['org_start'].' '.$row1['org_end'])?></td>
                        </tbody>
                        <?php endwhile?>  
                    </table>
                </div>
