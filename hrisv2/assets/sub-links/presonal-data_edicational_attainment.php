            ...
            <div class="container bg-light shadow-lg rounded py-3" style="margin-top: -220px; min-height: 220px;">
                ...
                ?>
                    <ul class="nav nav-pills nav-fill gap-2">
                        ...
                        <li class="nav-item border border-primary rounded">
                            <a href="/hrisv2/assets/sub-links/presonal-data_edicational_attainment.php?emp_id=<?php echo $id?>" class="nav-link active">Educational Attainment</a>
                        </li>
                        <li class="nav-item border border-primary rounded">
                ...
                if ($_SERVER['REQUEST_METHOD'] == 'POST') {
                    ...
                    $yr_graduated = $_POST['yr_graduated'];
                    $earned = $_POST['earned'];
                    $scholar_received = $_POST['scholar_received'];

                    if (isset($_POST['graduate']) && $_POST['graduate'] == '1') {
                        $graduate = 1;
                    } elseif (isset($_POST['graduate']) && $_POST['graduate'] == '2') {
                        $graduate = 2;
                    }else{
                        $graduate = 0;
                    }

                    $sql_grad = "SELECT grad_no FROM graduates WHERE grad_name = '$level'";
                    $result_grad = mysqli_query($conn, $sql_grad);
                    if (mysqli_num_rows($result_grad) > 0) {
                    $row_grad = mysqli_fetch_assoc($result_grad);
                    $grad_no = $row_grad['grad_no'];
                    } else {
                    die("level not found");
                    }

                    $sqlinsert = "INSERT INTO `employee_educations` (`grad_no`,`eeduc_course`,`eeduc_school`,`eeduc_school_address`,`eeduc_month`, `eeduc_yr_graduated`,`eeduc_units`,`eeduc_period_start`,`eeduc_period_end`,`eeduc_honor`, `eeduc_graduate`,`eeduc_encode_date`, `user_no`) 
                    SELECT '$grad_no','$course','$school','$address','$month_graduated','$yr_graduated','$earned','$start','$end','$scholar_received','$graduate',NOW(), b.user_no 
                    FROM `users` b
                    WHERE b.user_id='$id'";
                    ...
                    <form class="row justify-content-center needs-validation mb-5" method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]) . '?emp_id=' . $id;?>">
                        ...
                        <div class="mb-3 col-8">
                            ...
                            <div class="input-group">
                                <select class="form-select" id="salary-grade" name="level">
                                    <option selected value="">Select Level</option>
                                    <?php
                                    $result = mysqli_query($conn, "SELECT * FROM graduates WHERE grad_delete=0");
                                    while ($row = mysqli_fetch_assoc($result)) {
                                        $select = $row["grad_name"];
                        ...
                        <div class="mb-3 col-8">
                            ...
                            <input class="form-check-input" type="radio" name="graduate" value="1" id="flexRadioDefault1">
                            <label class="form-check-label" for="flexRadioDefault1">
                                Masteral
                            </label>
                            <input class="form-check-input" type="radio" name="graduate" value="2" id="flexRadioDefault1">
                            <label class="form-check-label" for="flexRadioDefault1">
                                Doctoral
                            </label>
                            <input class="form-check-input" type="radio" name="graduate" value="0"  id="flexRadioDefault1">
                            <label class="form-check-label" for="flexRadioDefault1">
                                Others
                            </label>
                        ...
                        <div class="mb-3 col-8">
                            <label for="simpleinput" class="form-label">Period of Attendance:</label>
                            <input type="text" class="form-control" name="start" placeholder="Year start">
                            <input type="text" class="form-control" name="end" placeholder="Year end">
                        </div>
                        <div class="mb-3 col-8">
                            <label for="simpleinput" class="form-label">Year Graduated:</label>
                            <div class="input-group">
                                <select class="input-group-text" id="salary-grade" name="month_graduated">
                                ...
                                <input type="text" name="yr_graduated" class="form-control">
                            </div>
                        </div>
                        <div class="mb-3 col-8">
                            <label for="simpleinput" class="form-label">Earned Units:</label>
                            <input type="text" class="form-control" name="earned">
                            <small>if not yet graduated</small>
                        </div>
                        <div class="mb-3 col-8">
                            <label for="simpleinput" class="form-label">Scholarship/Academic Honor Received:</label>
                            <input type="text" class="form-control" name="scholar_received">
                        </div>
                        <div class="mb-3 col-8">
                    ...
                    <div class="table">
                        ...
                        <table id="data-table" class="table table-hover mt-5">
                            <thead class="text-white bg-primary">
                                ...
                                <th>Units Earned(if not graduated)</th>
                                <th>Year Graduated</th>
                                <th>Scholarship/Academic Honor Received</th>
                            </thead>
                            <?php 
                            include_once 'connection.php';
                            $sql1 = "SELECT * FROM `users` LEFT JOIN employee_educations ON users.user_no = employee_educations.user_no LEFT JOIN graduates ON employee_educations.grad_no = graduates.grad_no WHERE user_id='$id';";
                            $res1 = $conn->query($sql1);
                            while($row1=$res1->fetch_assoc()):
                            ?>
                            <tbody>
                                <tr>
                                    <td><?php echo $row1['grad_name']?></td>
                                    <td><?php echo $row1['eeduc_school']?></td>
                                    <td><?php echo $row1['eeduc_course']?></td>
                                    <td><?php echo trim($row1['eeduc_period_start'].'-'.$row1['eeduc_period_end'])?></td>
                                    <td><?php echo $row1['eeduc_units']?></td>
                                    <td><?php echo $row1['eeduc_yr_graduated']?></td>
                                    <td><?php echo $row1['eeduc_honor']?></td>
                                </tr>
                            </tbody>
                            <?php endwhile?>
                        </table>
