    ...
    <div class="content-container" id="collapse">
            ...
            <ul class="nav nav-pills flex-column mb-auto d-block">
                <li id="first">
                    ...
                    <ul class="sub-nav nav nav-pills flex-column mb-auto d-block ms-5">
                        ...
                        <li>
                            <a href="/hrisv2/assets/dropdown-files/assign-panels.php" class="nav-link active text-white">Assign Panels</a>
                        </li>
                        <li>
        ...
        <section class="content" style="height: 100vh;">
            <div class="header bg-primary p-3">
                <p class="fs-3 text text-white">Assign Panels</p>
                <ul class="nav nav-pills">
                    <li>
                        <a href="/hrisv2/assets/dropdown-files/assign-panels.php" class="nav-link text-white">By Campus</a>
                    </li>
                    <li>
                        <a href="/hrisv2/assets/sub-links/by-discipline.php" class="nav-link bg-white">By Disciplines</a>
                    </li>
                </ul>
            </div>
            <div class="container bg-light shadow-lg rounded py-3" style="margin-top: -160px;">
                <form class="row justify-content-center needs-validation mb-5 pt-5" novalidate>
                    <div class="mb-3 col-8">
                        <label for="assessment" class="form-label">Assessment Type:</label>
                        <select class="form-select" id="assessment" required>
                            <option selected disabled value="">Select one</option>
                            <?php
                            include_once 'connection.php';
                            $result = mysqli_query($conn, "SELECT * FROM employee_applicants_assessments WHERE ass_delete=0");
                            while ($row = mysqli_fetch_assoc($result)) {
                                $select = htmlentities($row["ass_title"]);
                                echo "<option value=\"$select\">$select</option>";
                            }
                            ?>
                        </select>
                        <div class="invalid-feedback">Please provide a valid Assessment Type!</div>
                    </div>
                    <div class="mb-3 col-8">
                        <label for="assessment" class="form-label">Discipline:</label>
                        <select class="form-select" id="assessment" required>
                            <option selected disabled value="">Select one</option>
                            <?php
                            $result = mysqli_query($conn, "SELECT * FROM employee_applicants_discipline WHERE discipline_delete=0");
                            while ($row = mysqli_fetch_assoc($result)) {
                                $select = htmlentities($row["discipline_title"]);
                                echo "<option value=\"$select\">$select</option>";
                            }
                            ?>
                        </select>
                        <div class="invalid-feedback">Please provide a valid Discipline!</div>
                    </div>
                    <div class="mb-3 col-8">
                        ...
                        <select class="form-select" id="type" required>
                            ...
                            <?php
                            $result = mysqli_query($conn, "SELECT * FROM employee_applicants_panel_types WHERE ptype_delete=0");
                            while ($row = mysqli_fetch_assoc($result)) {
                                $select = htmlentities($row["ptype_title"]);
                                echo "<option value=\"$select\">$select</option>";
                            }
                            ?>
                        </select>
                    ...
                    <div class="mb-3 col-8">
                        <label for="date" class="form-label">Employee ID:</label>
            ...
            <div class="table">
                <table id="data-table" class="table table-hover mt-5">
                    ...
                    <?php
                    include_once 'connection.php';
                            $sql="select * from employee_applicants_panels 
                            join employee_applicants_discipline on employee_applicants_panels.discipline_no = employee_applicants_discipline.discipline_no 
                            join employee_applicants_assessments on employee_applicants_panels.ass_no = employee_applicants_assessments.ass_no 
                            join positions_assessments on employee_applicants_panels.ass_no = positions_assessments.ass_no 
                            join positions on positions_assessments.position_no = positions.position_no;";
                        ...
                        ?>
                        <tr>
                            <td><?php echo htmlentities($row['discipline_title']) ?></td>
                            <td><?php echo htmlentities($row['ass_title']) ?></td>
                            <td><?php echo htmlentities($row['panel_schedule']) ?></td>
                            <td><?php echo htmlentities($row['panel_type']) ?></td>
                            <td><?php echo htmlentities($row['position_title']) ?></td>
                        </tr>
                        <?php endwhile?>
                    </tbody>
                </table>
