            ...
            <div class="container bg-light shadow-lg rounded py-3" style="margin-top: -220px; min-height: 220px;">
                ...
                ?>
                    <ul class="nav nav-pills nav-fill gap-2">
                        ...
                        <li class="nav-item border border-primary rounded">
                            <a href="/hrisv2/assets/sub-links/presonal-data_job-skills-and-hobbies.php?emp_id=<?php echo $id?>" class="nav-link active">Job Skills and Hobbies</a>
                        </li>
                        <li class="nav-item border border-primary rounded">
                ...
                if($_SERVER['REQUEST_METHOD'] == 'POST'){
                    ...
                    $sql_job = "SELECT job_no FROM job_skills WHERE job_name = '$skill'";
                    ...
                    $sql_jsl = "SELECT jsl_no FROM job_skills_level WHERE jsl_name = '$level'";
                    $result_jsl = mysqli_query($conn, $sql_jsl);
                    if (mysqli_num_rows($result_jsl) > 0) {
                    $row_jsl = mysqli_fetch_assoc($result_jsl);
                    $jsl_no = $row_jsl['jsl_no'];
                    } else {
                    die("level not found");
                    }

                    $sqlinsert = "INSERT INTO `employee_job_skills`(`job_no`, `jsl_no`, `ejs_encode_date`, `user_no`) 
                    SELECT '$job_no','$jsl_no',NOW(), b.user_no 
                    FROM `users` b
                    WHERE b.user_id='$id'";
                ...
                <form class="row justify-content-center needs-validation mb-5" method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]) . '?emp_id=' . $id;?>">
                    <div class="mb-3 col-8">
                        ...
                        <div class="input-group">
                            <select class="form-select" id="salary-grade" name="skills" required>
                                <option selected disabled value=""></option>
                                <?php
                                $result = mysqli_query($conn, "SELECT * FROM job_skills");
                    ...
                    <div class="mb-3 col-8">
                        ...
                        <select class="form-select" id="salary-grade" name="level" required>
                            <option selected disabled value=""></option>
                            <?php
                                $result = mysqli_query($conn, "SELECT * FROM job_skills_level");
                                while ($row = mysqli_fetch_assoc($result)) {
                                    $select = $row["jsl_name"];
                                    echo "<option value=\"$select\">$select</option>";
                                }
                ...
                <div class="table">
                    ...
                    <table id="data-table" class="table table-hover mt-5">
                        ...
                        <?php 
                        $sql1 ="SELECT * FROM users a LEFT JOIN employee_job_skills b ON a.user_no = b.user_no LEFT JOIN job_skills c ON b.job_no = c.job_no LEFT JOIN job_skills_level d ON b.jsl_no = d.jsl_no WHERE user_id='$id';";
                        $res1 = $conn->query($sql1);
                        while($row1=$res1->fetch_assoc()):
                        ?>
                        <tbody>
                            <td><?php echo $row1['job_name']?></td>
                            <td><?php echo $row1['jsl_name']?></td>
                        </tbody>
                        <?php endwhile?>  
                    </table>
                </div> 
