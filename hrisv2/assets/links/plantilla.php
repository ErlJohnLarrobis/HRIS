            ...
            <div class="container bg-light shadow-lg rounded py-3">
                ...
                if ($_SERVER['REQUEST_METHOD'] == 'POST') {
                    if(isset($_POST['delete-button'])){
                        ...
                        $sqldelete = "UPDATE positions SET position_delete=1 WHERE position_no=$iddelete;";
                        ...
                        $stmt = $conn->prepare("INSERT INTO positions (sg_no, position_code, position_title, position_type, position_encode_date) VALUE (?,?,?,?,NOW())");

                        $stmt->bind_param("isss",$sg,$code,$name,$type);
                ...
                <div class="table">
                    <table id="data-table" class="table table-hover mt-5">
                        ...
                        <tbody>
                            <?php                       
                            $sql = "SELECT * FROM positions WHERE position_delete=0";
                            ...
                            ?>
                            <tr>
                            <td><?php echo htmlentities($row['position_code']); ?></td>
                            <td class='clearfix'>
                                <?php 
                                $position_title = htmlentities($row['position_title']); 
                                $position_type = htmlentities($row['position_type']);
                                echo "<p class='float-start'>$position_title</p><p class='float-end'>$position_type</p>";
                                ?>
                            </td>
                            <td><?php echo htmlentities($row['sg_no']); ?></td>
                                <td class="d-flex">
                                    <button class="btn btn-info"><a class="text-decoration-none text-dark" href="/hrisv2/assets/data-modifier/plantilla-update.php?id=<?php echo $row['position_no']?>">Edit</a></button>&nbsp;
                                    <form method='post' action='<?php echo $_SERVER['PHP_SELF']; ?>'><input type='hidden' name='delete-button' value='<?php echo $row['position_no'] ?>'><input class="btn btn-danger" type='submit' value='Delete'></form>
                                </td>
                            </tr>
                            <?php endwhile; ?>
                        </tbody>
