        ...
        <section class="content" style="height: 100vh;">
            ...
            <?php 
                if ($_SERVER['REQUEST_METHOD'] == 'POST') {
                    ...
                    $sqlinsert = "UPDATE employee_awards SET at_no='$at_no', campus_no='$campus_no', award_title='$title', award_inactive='$status', award_encode_date=NOW() WHERE award_no=$id";
                    if(mysqli_query($conn, $sqlinsert)){
                        ...
                        echo '<script>
                            setTimeout(function() {
                            window.location.href = "/hrisv2/assets/links/rewards-awards.php";
                            }, 1500);
                            </script>';
                        exit;
                    } else {
                ...
                if(isset($_GET['id'])){
                    $getid = $_GET['id'];
                    $getsql = "SELECT * FROM `employee_awards` JOIN employee_awards_types ON employee_awards.at_no = employee_awards_types.at_no JOIN campuses ON employee_awards.campus_no = campuses.campus_no WHERE award_no=$getid;";
                    $result = mysqli_query($conn, $getsql);
                    $row = mysqli_fetch_assoc($result);
                    $gettype = $row['at_title'];
                    $getcampus = $row['campus_title'];
                    $gettitle = $row['award_title'];
                    $getstatus = $row['award_inactive'];
                ...
                <form class="row justify-content-center needs-validation mb-5 pt-5" action="" method="POST" novalidate>
                    ...
                    <div class="mb-3 col-8">
                        ...
                        <select class="form-select" id="salary-grade" name="campus" required>
                            <option selected value="<?php echo $getcampus ?>"><?php echo $getcampus ?></option>
                        <?php
                    ...
                    <div class="mb-3 col-8">
                        ...
                        <input type="text" id="code" class="form-control" name="title" value="<?php echo $gettitle ?>" required>
                        <div class="invalid-feedback">
                    ...
                    <div class="mb-3 col-8">
                        ...
                        <div>
                            <input class="form-check-input" type="radio" name="open/close" value="0" <?php echo $getstatus == 0 ? "checked" : ""; ?>  required>
                            <label for="active" class="form-check-label">Open</label>
                            <input class="form-check-input" type="radio" name="open/close" value="1" <?php echo $getstatus == 1 ? "checked" : ""; ?> required>
