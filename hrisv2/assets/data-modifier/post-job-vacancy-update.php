        ...
        <section class="content">
            ...
            include_once 'connection.php';
                if ($_SERVER['REQUEST_METHOD'] == 'POST') {
                    ...
                    $sql4 = "UPDATE `employee_applicants_vacancy` SET `position_no`='$position_no',`tenur_no`='$tenur_no',`campus_no`='$campus_no',`discipline_no`='$discipline_no',
                    `vacancy_subtitle`='$subtitle',`vacancy_slots`='$slot',`vacancy_education`='$education',`vacancy_experience`='$exp',`vacancy_eligibility`='$elig',
                    `vacancy_assign`='$place',`vacancy_training`='$training',`vacancy_date_due`='$duedate',`vacancy_encode_date`=NOW(),
                    `vacancy_inactive`='$active' WHERE vacancy_no=$id";
                    if (mysqli_query($conn, $sql4)) {
                        ...
                        echo '<script>
                            setTimeout(function() {
                            window.location.href = "/hrisv2/assets/dropdown-files/post-job-vacancy.php";
                            }, 1500);
                            </script>';
                        exit;
                    } else {
                ...
                if(isset($_GET['id'])) {
                    $id = $_GET['id'];
                    $query = "SELECT * FROM `employee_applicants_vacancy` JOIN campuses on employee_applicants_vacancy.campus_no = campuses.campus_no JOIN positions on employee_applicants_vacancy.position_no = positions.position_no JOIN tenureships on employee_applicants_vacancy.tenur_no = tenureships.tenur_no JOIN employee_applicants_discipline on employee_applicants_vacancy.discipline_no = employee_applicants_discipline.discipline_no WHERE vacancy_no=$id;";
                    $result = mysqli_query($conn, $query);
                    $row = mysqli_fetch_assoc($result);
                    $uposition = $row['position_title'];
                    $udiscipline = $row['discipline_title'];
                    $usubtitle = $row['vacancy_subtitle'];
                    $ucampus = $row['campus_title'];
                    $ustatus = $row['tenur_name'];
                    $ueducation = $row['vacancy_education'];
                    $uexperience = $row['vacancy_experience'];
                    $ueligibility = $row['vacancy_eligibility'];
                    $utraining = $row['vacancy_training'];
                    $uplace = $row['vacancy_assign'];
                    $uslot = $row['vacancy_slots'];
                    $uduedate = $row['vacancy_date_due'];
                    $uactive = $row['vacancy_inactive'];
                  }
            ?>
            <form class="row justify-content-center needs-validation mb-5 pt-5" action="" method="POST" novalidate>
            
                    <div class="mb-3 col-8">
                        <input type="hidden" name="id" value="<?php echo $id ?>">
                        <label for="example-select" class="form-label">Position:</label>
                        <select class="form-select" id="position" name="position" required>
                            <option selected value="<?php echo $uposition ?>"><?php echo $uposition ?></option>
                            <?php
                    ...
                    <div class="mb-3 col-8">
                        ...
                        <select class="form-select" id="discipline" name="discipline" required>
                            <option selected value="<?php echo $udiscipline ?>"><?php echo $udiscipline ?></option>
                            <option value="N/A">N/A</option>
                            <?php
                            $result = mysqli_query($conn, "SELECT * FROM employee_applicants_discipline");
                            while ($row = mysqli_fetch_assoc($result)) {
                    ...
                    <div class="mb-3 col-8">
                        ...
                        <input type="text" id="sub-title" class="form-control" name="subtitle" required value="<?php echo $usubtitle ?>">
                    ...
                    <div class="mb-3 col-8">
                        ...
                        <option selected value="<?php echo $ucampus?>"><?php echo $ucampus?></option>
                            <?php
                    ...
                    <div class="mb-3 col-8">
                        ...
                        <select class="form-select" id="employmentStatus" name="status" required>
                            <option value="N/A">N/A</option>
                            <option selected value="<?php echo $ustatus?>"><?php echo $ustatus?></option>
                            <?php
                            $result = mysqli_query($conn, "SELECT * FROM tenureships");
                            while ($row = mysqli_fetch_assoc($result)) {
                    ...
                    <div class="mb-3 col-8">
                        ...
                        <input type="text" id="education" class="form-control" name="education" value="<?php echo $ueducation?>" required>
                        <div class="invalid-feedback">Please provide a valid Education !</div>                 
                    </div>
                    <div class="mb-3 col-8">
                        <label for="Experience" class="form-label">Experience:</label>
                        <input type="text" id="Experience" class="form-control" name="experience" value="<?php echo $uexperience?>" required>
                        <div class="invalid-feedback">Please provide a Work Experienced !</div>                 
                    </div>
                    <div class="mb-3 col-8">
                        <label for="eligibility" class="form-label">Eligibility:</label>
                        <input type="text" id="eligibility" class="form-control" name="eligibility" value="<?php echo $ueligibility?>" required>
                        <div class="invalid-feedback">Please provide a valid Eligibility !</div>                 
                    </div>
                    <div class="mb-3 col-8">
                        <label for="training" class="form-label">Training:</label>
                        <input type="text" id="training" class="form-control" name="training" value="<?php echo $utraining?>" required>
                    ...
                    <div class="mb-3 col-8">
                        ...
                        <input type="text" id="placeofAssignment" class="form-control" name="placeassign" value="<?php echo $uplace?>" required>
                    ...
                    <div class="mb-3 col-8">
                        ...
                        <input type="number" id="numberofSlot" class="form-control" name="slot" value="<?php echo $uslot?>" required>
                    ...
                    <div class="mb-3 col-8">
                        ...
                        <input type="date" id="cutoffDate" class="form-control" name="cutoff" value="<?php echo $uduedate?>" required>
                        <div class="invalid-feedback">Please provide a valid Cut-off Date !</div>                 
                    </div>
                    <div class="mb-3 col-8 d-flex">
                        <div class="form-check">
                            <input type="radio" class="form-check-input" id="validationFormCheck2" name="radio-stacked" value="0" <?php echo $uactive == 0 ? "checked" : ""; ?> required>
                            <label class="form-check-label" for="validationFormCheck2">Active</label>
                        </div>
                        <div class="form-check mb-3">
                            <input type="radio" class="form-check-input" id="validationFormCheck3" name="radio-stacked" value="1" <?php echo $uactive == 1 ? "checked" : ""; ?> required>
